...
- 已上线订单列表：支持订单号、用户ID、邮箱、渠道、状态、时间范围筛选
- 列表字段：订单号、用户ID、邮箱、渠道（Google Play/兑换码/管理员）、积分、金额、状态、创建/完成时间
- API：`GET /api/v1/admin/purchases`，分页返回 `purchases/total/page/size`
...
| 订单 | `GET` | `/api/v1/admin/purchases` | 支付记录/兑换记录列表 | `page,size,platform,status,installation_id,email,order_id,date_from,date_to` | `{ purchases, total }` |
| 订单 | `GET` | `/api/v1/admin/purchases/{id}` | 订单详情 | Path: 订单 ID | `{ purchase, transactions }` |
