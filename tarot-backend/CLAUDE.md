...
| GET  | `/api/v1/admin/purchases` | 管理后台订单列表（含渠道、状态筛选） | ✅ 已实现 |
| GET  | `/api/v1/admin/purchases/{id}` | 管理后台订单详情 + 关联流水 | ✅ 已实现 |

### 支付与身份兼容（重要）
- 通道：Google Play（海外/有 Play 服务）+ 兑换码（中国大陆/无 Play 服务、离线场景）。
- 标识：客户端在所有支付相关请求中必须传 `installation_id`；若可用，建议附带 `email` 以进行账号绑定。
- 校验接口：`POST /api/v1/payments/google/verify` 建议接受 `email`（可选），在校验成功后将邮箱写入/更新到 `users.email`。
- 数据一致：`Purchase.product_id` 建议首版即按“积分数”（5/10/20/50/100）存储，与兑换码一致，简化统计与对账。
- 管理后台用户详情中的 `recent_transactions` 已携带 `platform` 和 `order_id` 字段，用于区分 Google Play / 兑换码 / 管理员调整来源。

### 应用发布接口

| 方法 | 路径 | 说明 | 状态 |
| ---- | ---- | ---- | ---- |
| GET | `/api/v1/app-release/latest` | 公开查询最新上线的 APK 元数据（无需登录） | ✅ 新增 |
| GET | `/api/v1/admin/app-release/latest` | 管理后台查询最新版本信息 | ✅ 新增 |
| POST | `/api/v1/admin/app-release` | 管理员上传 APK 并发布新版本 | ✅ 新增 |
| GET | `/api/v1/admin/app-release/history` | （可选）历史版本分页列表 | ✅ 新增 |
...
8. **app_releases** - 应用发布记录（存储 APK 文件元数据、校验值、下载路径）
