...
class Settings(BaseSettings):
    ...
    def validate_runtime_secrets(self) -> None:
        """Require sensitive settings outside explicit debug mode."""
        if self.DEBUG:
            return

        missing = [
            name
            for name in ("JWT_SECRET_KEY", "ADMIN_PASSWORD", "WEBHOOK_SECRET_KEY")
            if not getattr(self, name)
        ]
        if missing:
            joined = ", ".join(missing)
            raise ValueError(f"Missing required environment settings: {joined}")
